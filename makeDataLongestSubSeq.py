...
def genTrials(n_num, m_num, r_num,storage, trialnum, filename,option):
	...
	for m_i in xrange(3,m_num):
		for r_i in xrange(0,min(m_i,r_num)-2): #can only remove up to m_i - 2 edges b/c there are only m_i - 1 edges total
			trialstorage = np.empty(trialnum,int)
			for trials in xrange(0,trialnum):
				
				# trialstorage[trials]=gen.Graph(n=n_i,m=m_i,option=option,remove=1).emitLongestLength()
				g.createAdjacencyandMutation(m_i,option,remove=r_i)
				
				trialstorage[trials]=g.emitLongestLength()
				#Data is now stored as  r_i,m_i
				storage[r_i][m_i] = float(np.sum(trialstorage))/(float(trialnum) * (m_i*2-2-r_i*2))
	#print storage
...
if __name__ == '__main__':
	...
	trialnum = 2500
	filename = 'option%sLongestSubSeqRatio' %(option)
